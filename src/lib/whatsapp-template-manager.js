 ...
 */
export async function fetchUniqueTemplates() {
  try {
 ...
 */
export async function removeDuplicateTemplates() {
  try {
    // Get all templates
    const records = await pb.collection('whatsapp_templates').getFullList({
 ...
 */
export function validateTemplateVariables(templateContent) {
 ...
 */
export function getOrderDeliveredDefaultTemplate() {
 ...
 */
export async function ensureOrderDeliveredTemplateHasFeedbackLink() {
  try {
    ...
    if (existingTemplates.items.length > 0) {
      const template = existingTemplates.items[0];
    ...
    console.error('Error ensuring ORDER_DELIVERED has feedback link:', error);
    return false;
  }
} 